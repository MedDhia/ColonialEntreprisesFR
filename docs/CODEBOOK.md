...
|---|---|---|
| `n_firms`, `n_interlocks` | 5,959 / 78,530 | Size of the interlock graph. |
...
| `mean_path_length` | 3.2346 | Sampled from 200 sources, seed 11. |
| `longest_path_observed` | 9 | In the same sample; a lower bound on the diameter, not the diameter. |
| `degree_assortativity` | 0.1559 | Positive: hubs attach to hubs. |
| `average_clustering`, `transitivity` | 0.4461 / 0.2396 | Local and global clustering. Both are inflated by construction — a bipartite projection turns every board into a clique. |
| `n_communities`, `modularity` | 65 / 0.4819 | Louvain, seed 7, weighted. |
| `cross_territory_share` | 0.6755 | Interlocks joining two different territories. |
