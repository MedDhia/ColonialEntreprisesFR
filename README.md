...
| Person → company ties | ~28,000 dated observations |
| Distinct people | ~13,000 |
| Companies | ~7,300 (including firms known only from directory entries) |
| Company interlock edges | ~12,000 |
| Period covered | 1830s–1970s, densest 1914–1944 |

Counts are from a partial extraction run and will settle slightly higher once
all documents are processed; `data/processed/network_stats.csv` always holds
the current figures.
...
src/
  ...
  checks.py            87 assertions on the parsers and the built dataset
data/
...
Stage 2 is resumable and streams each PDF through memory, so the ~45 GB of
source material never lands on disk. Stages 3 and 4 are pure functions of the
text cache — that is where to iterate when changing coding rules.
...
- **A coverage gap beats a fabricated attribution.** Where the parser cannot
  ...
  excluded from the network (~10% of parsed ties) instead of being credited to
  the previous firm in the document.
